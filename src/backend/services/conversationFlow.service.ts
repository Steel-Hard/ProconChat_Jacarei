...
import { sessionRepository, SessionRepository, SessionStep } from "../repositories/session.repository"
...
export function createConversationFlowService({
    ...
    motor,
    hashPhone,
}: ConversationFlowDependencies): (input: ConversationFlowInput) => Promise<ConversationFlowResult> {
    async function startOver(sessionId: string, newSession: boolean): Promise<ConversationFlowResult> {
    ...
    async function handleAwaitingQuestion(
        ...
        return {
            sessionId,
            newSession: false,
            reply: { text: formatarRespostaFinal(resposta), step: "FINISHED" },
        }
    }

    return async ({ phone, text }: ConversationFlowInput): Promise<ConversationFlowResult> => {
        ...
        return handleAwaitingQuestion(session.id, session.currentCategoryId, text)
    }
}
